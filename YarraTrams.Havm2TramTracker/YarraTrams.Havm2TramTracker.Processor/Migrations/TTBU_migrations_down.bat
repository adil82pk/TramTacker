...
if "%UseActiveDirectory%"=="true" (
	...
	"../../packages/FluentMigrator.Console.3.2.1/net461/x64/Migrate.exe" /tag "TTBU" /conn "Data Source=%TestComparisonsDBHostName%;Persist Security Info=True;database=%TestComparisonsDBName%;Integrated Security=SSPI;" /provider sqlserver2012 /assembly "../bin/Debug/YarraTrams.Havm2TramTracker.Processor.exe" /verbose=true --task rollback
)

if "%UseActiveDirectory%"=="false" (
	echo "Running Havm2TramTracker TTBU DB migrations up using username/password..."
	"../../packages/FluentMigrator.Console.3.2.1/net461/x64/Migrate.exe" /tag "TTBU" /conn "Data Source=%TestComparisonsDBHostName%;Persist Security Info=True;User ID=%TestComparisonsDBUserName%;Password=%TestComparisonsDBPassword%;database=%TestComparisonsDBName%;" /provider sqlserver2012 /assembly "../bin/Debug/YarraTrams.Havm2TramTracker.Processor.exe" /verbose=true --task rollback
)
